...
public class UserDAO{
	...
	public List<UDVO> display(int i){
		...
		try{
				...
				ResultSet rs = st.executeQuery("SELECT s.sensor_id,t.temp,s.sensor_tag_value,s.sensor_status,max(t.time_stamp) as creation_time FROM iaasdb.temp_data t join iaasdb.sensor_master s on (s.sensor_id=t.sensor_id) where t.user_id="+i+" group by t.sensor_id");
...
public List<UDVO> display1(int i){
	...
	try{
			...
			//iaasdb.chzfjyaimyou.us-west-2.rds.amazonaws.com:3306
			Statement st = con.createStatement();
			ResultSet rs = st.executeQuery("SELECT s.sensor_id,t.temp,s.sensor_tag_value,s.sensor_status,t.time_stamp as creation_time FROM iaasdb.temp_data t join iaasdb.sensor_master s on (s.sensor_id=t.sensor_id) where t.user_id="+i+"");
			//+" group by t.sensor_id"
			while(rs.next()){
				UDVO udvo = new UDVO();
				udvo.setSensor_id(rs.getString("sensor_id"));
