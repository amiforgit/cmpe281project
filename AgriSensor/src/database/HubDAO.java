...
public class HubDAO {
	public List<HDVO> display(int hub_id){
		...
		try{
				...
				//iaasdb.chzfjyaimyou.us-west-2.rds.amazonaws.com:3306/iaasdb?useSSL=false
				Statement st = con.createStatement();
				ResultSet rs = st.executeQuery("SELECT * from iaasdb.sensor_master where hub_id="+hub_id+"");
				while(rs.next()){
					HDVO hdvo = new HDVO();
