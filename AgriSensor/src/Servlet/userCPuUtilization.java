...
public class userCPuUtilization extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String query = "SELECT sensor_id FROM iaasdb.sensor_master where user_id ="+user_id;
...
private ArrayList<ArrayList<Double>> getUtilizationData(ArrayList<String> sensor_ids, ArrayList<String> dates) throws ClassNotFoundException, SQLException {
		...
		for(int i=0; i<sensor_ids.size(); i++)
		{
			...
			String query = "SELECT date, AVG(cpu_data) as avgutil FROM iaasdb.utilization where sensor_id='"+sensor_ids.get(i)+"' group by CAST(date AS date) order by date DESC limit 10";
