...
public class Charts extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String q = "SELECT sensor_id FROM iaasdb.sensor_master where user_id="+id;
		...
		if(sensor_ids!=null)
		{
			try {
				...
				{
					String sensorname = "SELECT sensor_tag_value FROM iaasdb.sensor_master where sensor_id='"+sensor_ids.get(i)+"'";
	...
	private ArrayList<ArrayList<Double>> getTempDetails(ArrayList<String> sensor_ids, ArrayList<String> dates) throws ClassNotFoundException, SQLException {
		...
		for(int i=0; i<sensor_ids.size(); i++)
		{
			...
			String query = "SELECT time_stamp, AVG(temp) as avgtemp FROM iaasdb.temp_data where sensor_id='"+sensor_ids.get(i)+"' group by CAST(time_stamp AS date) order by time_stamp DESC limit 15";
