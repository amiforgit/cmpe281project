...
public class InvokeInstance {
	...
	public void createSensor(String user_id, String sensor_name, String lat, String lon) throws InterruptedException {
		...
		AmazonEC2 amazonEC2Client = new AmazonEC2Client(new BasicAWSCredentials("AKIAI3VYQX3UB3W6KOMA", "Z0HdxoiVABR/B0J0zw06heg/G8wdNYsze3rO/yHw"));
		//AKIAI3VYQX3UB3W6KOMA
		//Z0HdxoiVABR/B0J0zw06heg/G8wdNYsze3rO/yHw
...
//new AmazonEC2Client(new BasicAWSCredentials("AKIAI32NAJBWBXFWKXPQ", "fbQTu9ERdPKdkKFycmnDIgMpQ2ziphaiFIHjXUw6"));
		...
		runInstancesRequest.withImageId("ami-a9d276c9")  //ami-9f5efbff
                   .withInstanceType("t2.micro")  
                   .withMinCount(1)
                   .withMaxCount(1)
                   .withKeyName("AWSKey_Sam")  //class
                   
                   //.withKeyName("Hello_281")
                   .withSecurityGroups("DB-Security-Group"); //DB-Security-Group
...
while(true){
	...
	//ires.getReservations().get(0).getInstances().get(0).getTags().get(0).getValue();
		...
		try {
		//	Thread.sleep(50000);
			System.out.println("sensor_2:"+sensor_id+","+lat+","+lon+","+","+","+sensor_name);
		...
		System.out.println("sensor_2:"+sensor_id+","+lat+","+lon+","+status_new+","+","+sensor_name);
		String name="Name";
		...
		String qq1 = "SELECT no_of_server FROM iaasdb.hub_master where hub_id="+hub;
		int no_of_server = db.getNoodfSensor(qq1);
		no_of_server++;
		
		String qu = "UPDATE `iaasdb`.`hub_master` SET `no_of_server`='"+no_of_server+"' WHERE `hub_id`="+hub;
		db.insert(qu);
	...
	{
		int new_hub_id;
		String q = "SELECT  hub_id from iaasdb.hub_master where no_of_server<3";
		...
		if(check_hub_id==0)
		{
			String query = "INSERT INTO `iaasdb`.`hub_master` (`no_of_server`, `hub_status`) VALUES ('0', 'Running');";
			db.insert(query);
			String q1= "SELECT hub_id FROM iaasdb.hub_master";
